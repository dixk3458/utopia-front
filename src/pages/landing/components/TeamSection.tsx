...
export default function TeamSection() {
  // ✅ Fix: buttonClassName을 컴포넌트 내부로 이동 (파일 스코프 상수 → 컴포넌트 내부 상수)
  const buttonClassName = 'p-2 text-gray-400 hover:text-primary transition-colors';
    ...
    <section id="team" className="py-20 md:py-32 bg-white">
      <div className="flex flex-col items-center text-center mb-16">
        ...
        <p className="text-gray-500 text-base md:text-lg max-w-2xl mx-auto leading-relaxed">
          AI, 보안, 풀스택 개발 전문성을 가진 5명의 개발자가 모여 안전한 구독 파티 매칭 플랫폼을 구현했습니다.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {members.map((member, index) => (
          ...
          >
            ...
            <p className="text-sm font-semibold text-purple-600 mb-4">{member.role}</p>
            <p className="text-sm text-gray-500 mb-6 leading-relaxed px-4">{member.description}</p>
            ...
            <div className="flex items-center gap-4 pt-6 border-t border-gray-50 w-full justify-center">
              <button className={buttonClassName}><FiGithub size={18} /></button>
              <button className={buttonClassName}><FiLinkedin size={18} /></button>
              <button className={buttonClassName}><FiMail size={18} /></button>
            </div>
          </div>
        ))}
        ...
        <div className="bg-linear-to-br from-primary to-secondary rounded-2xl p-8 flex flex-col items-center justify-center text-center text-white shadow-xl shadow-blue-200">
          ...
          <p className="text-sm opacity-90 mb-6">이 프로젝트는 경진대회 출품을 위해 제작되었습니다.</p>
          <div className="space-y-2 text-sm font-medium">
            <p className="flex items-center justify-center gap-2">📅 개발 기간: 2026.01 - 2026.03</p>
            <p className="flex items-center justify-center gap-2">👥 팀 구성: 5명</p>
          </div>
        </div>
      </div>
    </section>
