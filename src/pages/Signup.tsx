...
export default function Signup() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const errorMsg = axiosError.response?.data?.detail || "회원가입에 실패했습니다.";
      alert(errorMsg);
    }
  };
  ...
  return (
    <div className="mx-auto mt-10 max-w-2xl rounded-xl border border-gray-100 bg-white p-10 shadow-lg">
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        {/* 이메일 */}
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-600">이메일</label>
        ...
        {isEmailVerified && <p className="text-xs text-green-600 font-medium ml-1">이메일 인증이 완료되었습니다.</p>}
        ...
        {/* 비밀번호 재확인 */}
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-600">비밀번호 재확인</label>
          <input
            type="password"
        ...
        {/* 닉네임 */}
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-600">닉네임</label>
          <div className="flex gap-2">
            <input
              name="nickname"
              type="text"
              ...
              required
            />
            <button type="button" onClick={handleCheckNickname} className="shrink-0 rounded-lg border border-gray-300 px-4 py-2 font-medium hover:bg-gray-50">
              중복검사
            </button>
          </div>
        </div>
        ...
        {/* 휴대폰 번호 (선택) */}
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-600">휴대폰 번호 (선택)</label>
          <input
            name="phone_number"
            type="tel"
